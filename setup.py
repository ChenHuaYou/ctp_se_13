...
include_dirs = [PREFIX+'ctp13', api_dir]
library_dirs = [api_dir]
ext_modules = []; package_data = []
for k,v in BUILD:
    extm = Extension(name='ctp13._'+k, language='c++',
        include_dirs=include_dirs, library_dirs=library_dirs,
        libraries=[v], sources=['ctp13/%s.%s'%(k,ext)],
    )
...
setup(
    name='ctp13', version=__version__, author=__author__,
    cmdclass=cmdclass, ext_modules=ext_modules,
    packages=['ctp13'], package_dir={'ctp13':'ctp13'}, package_data={'ctp13':package_data},
)
